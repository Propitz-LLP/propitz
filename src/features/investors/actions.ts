...
// password is never shown — the investor sets their own via "Change / Reset
...
export async function createInvestorAction(formData: FormData) {
  ...
  // Register via signUp (not admin.createUser) so Supabase sends its built-in
  // account email — the same one investors get when they self-register from the
  // login screen. A fresh, non-persisting client keeps the admin's own session
  // untouched (signUp would otherwise swap the session to the new user).
  const { createClient: createSupabaseClient } = await import('@supabase/supabase-js')
  const authClient = createSupabaseClient(config.supabase.url, config.supabase.publishableKey, {
    auth: { persistSession: false, autoRefreshToken: false },
  })
  const { data, error } = await authClient.auth.signUp({ email, password: tempPassword })
  if (error || !data.user) {
